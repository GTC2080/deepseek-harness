...
macOS 桌面端每次都会在不同端口启动 loopback host，因此 `SessionRuntime` 会先从经校验且有大小限制的 Session selection cookie，为该端口独立的 `localStorage` 播种，并把后续选择变化同步回应用 WebView 的 loopback 主机级 cookie。普通浏览器继续保持既有的同源本地行为。桌面端首次迁移尚无持久 selection 时，`WorkspaceRuntime` 会打开最近活跃 Workspace 中最近更新、未归档的非空白 Session，而不会制造一条空白启动行。
