...
在 macOS 与 Windows 上，loopback 端口都会随每次启动变化。因此，桌面客户端会在 Web 常规启动前恢复上次选中的 Session，并通过应用 WebView 的主机级存储同步后续选择。首次迁移还没有持久桌面选择时，会打开最近更新的非空白 Session，而不会创建一个只在启动时出现的空白草稿。Session Log 下载仍由 WebView 下载管理器处理；原生壳会把完成状态和实际保存文件名反馈给现有 Web 弹窗。
