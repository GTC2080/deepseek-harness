...
Rust unit tests accept the expected readiness origin and reject HTTPS, `localhost`, missing ports, non-root paths, and credentials. macOS-specific tests prove that the native download bridge accepts only loopback Session exports and JSON-escapes filenames before dispatch. Client regressions cover invalid or oversized selection cookies, cross-port restoration, first-migration fallback, browser isolation, native download completion, collision-safe filenames, and native download failure. Two consecutive development launches on different loopback ports restore the same real Session without showing a startup-only New Session row.
