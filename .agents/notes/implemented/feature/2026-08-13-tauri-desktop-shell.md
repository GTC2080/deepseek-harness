...
The Win32 native folder picker keeps its blocking COM conversation in a dedicated child process. Ordinary Node deployments launch the sibling `lib/worker.cjs`; inside the closed SEA, `process.execPath` is the sidecar itself rather than a generic Node interpreter, so the picker relaunches it with the exact private `--dsh-internal-win32-dialog-worker` argument. The packaged CLI accepts that mode only under `process.pkg` and imports the shipped worker entry, whose existing non-empty title and IPC-channel checks remain the final dispatch boundary.
...
The same build also relaunches the compiled executable through the packaged dialog-worker mode over a real Node IPC channel. Windows must report a positive native thread id in its `showing` message before the verifier terminates the modal child; other build hosts must reach the worker and return its structured unsupported-native error. This closes the gap where source tests passed under a system Node executable while an installed SEA silently re-entered the main CLI instead of running `worker.cjs`.
...
The Windows 2025 native runner builds both x64 NSIS and MSI installers. Its source-plane COM smoke and packaged-worker IPC handshake cover the two distinct launch paths; an installed-app manual check remains the final confirmation that the visible chooser opens and returns a selected workspace on the target user's desktop.
